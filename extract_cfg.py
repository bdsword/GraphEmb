...
def extract_cfg(file_path):
    ...
    for f in r2agent.cmd('afl').split('\n'):
        ...
        func_name = items[-1]
        func_addr = items[0]
        if not re.search("(sym\._)|(sym\.imp)|(sym\.mingw)|(sym\..*cxx::)|(sym\..*std::)|(sym\.register_tm_clones)|(sym\.deregister_tm_clones)|(call_weak_fn)|(operatordelete)|(operatornew)", func_name) and re.search("sym\.", func_name):
